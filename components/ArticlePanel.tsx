...
import { Article, themes, getTheme, getRelated } from '@/lib/data'
...
function Divider() {
  return <div style={{ height: 1, background: 'var(--rule)', margin: '20px 0' }} />
}
          ...
          <motion.aside key="panel"
            ...
            <div className="flex-1 overflow-y-auto">
              <div className="px-6 py-5 space-y-0">
                ...
                {/* quote */}
                <blockquote className="pl-4 py-1 text-[13px] leading-relaxed italic mb-5"
                  style={{ borderLeft: `2px solid var(--rule-2)`,
                    fontFamily: 'var(--font-serif)', color: 'var(--ink-2)' }}>
                  {article.quote}
                </blockquote>
                ...
                <ul className="space-y-3 mb-0">
                  {article.keyPoints.map((pt, i) => (
                    <li key={i} className="flex gap-3">
                      <span className="text-[9px] font-mono mt-1 shrink-0 tabular-nums"
                        style={{ color: 'var(--ink-3)' }}>
                        {String(i + 1).padStart(2, '0')}
                      </span>
                      <span className="text-[12px] leading-relaxed" style={{ color: 'var(--ink)' }}>
                        {pt}
                      </span>
                    </li>
                  ))}
                ...
                {/* related theories */}
                <SectionLabel>关联理论</SectionLabel>
                ...
                {/* analogies */}
                <SectionLabel>他山之石</SectionLabel>
                <div className="space-y-4 mb-0">
                  {article.analogies.map((an, i) => (
                    <div key={i} className="pl-3" style={{ borderLeft: '1.5px solid var(--rule)' }}>
                      <p className="text-[11px] leading-relaxed italic mb-1"
                        style={{ color: 'var(--ink)', fontFamily: 'var(--font-serif)' }}>
                        "{an.quote}"
                      </p>
                      <p className="text-[9px] font-mono mb-1.5" style={{ color: 'var(--ink-3)' }}>
                        — {an.from}
                      </p>
                      <p className="text-[11px] leading-relaxed" style={{ color: 'var(--ink-2)' }}>
                        {an.note}
                ...
                {/* extensions */}
                <SectionLabel>发散思考</SectionLabel>
                <ul className="space-y-3 mb-0">
                  {article.extensions.map((ext, i) => (
                    <li key={i} className="flex gap-3">
                      <span className="text-[9px] font-mono mt-0.5 shrink-0" style={{ color: 'var(--ink-3)' }}>
                        →
                      </span>
                          ...
                          return (
                            <button key={rel.id} onClick={() => onSelect(rel)}
                              ...
                              <div className="flex items-start justify-between gap-2">
                                ...
                                <span className="text-[9px] font-mono shrink-0" style={{ color: 'var(--ink-3)' }}>
                                  {rel.dateLabel}
                                </span>
                              </div>
                              <div className="mt-1.5 flex gap-1.5 flex-wrap">
                                {shared.map((t) => (
                                  <span key={t.id} className="text-[8px] font-mono px-1.5 py-0.5"
                                    style={{ color: t.color, background: t.color + '15', borderRadius: 2 }}>
                                    {t.label}
                                  </span>
                                ))}
                              </div>
            ...
            {/* ── footer ── */}
            {article.url && (
              <div className="px-6 py-4 flex-shrink-0" style={{ borderTop: '1px solid var(--rule)' }}>
